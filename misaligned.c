...
#define MiB (1024 * KiB)
#define GiB (1024 * MiB)
#define U64_8X_U8(data) (((unsigned long int)(data) << 56) + \
    ((unsigned long int)(data) << 48) + ((unsigned long int)(data) << 40) + \
    ((unsigned long int)(data) << 32) + ((unsigned long int)(data) << 24) + \
    ((unsigned long int)(data) << 16) + ((unsigned long int)(data) << 8) + \
    ((unsigned long int)(data)))

#define AS_U8(addr)         ((unsigned char*)(addr))
#define AS_ANY(addr)        ((void*)(addr))
#define AS_U64(addr)        ((unsigned long int*)(addr))
#define U8(value)           ((unsigned char)(value))
#define ALIGN               sizeof(unsigned long int)
#define MASK                (ALIGN - 1)
#define ADDR_AS_NUM(addr)   ((unsigned long int)(addr))
#define MISALIGN(addr)      (ADDR_AS_NUM(addr) & MASK)
#define ALIGNED_ADDR(addr)  ((ADDR_AS_NUM(addr) + ALIGN) & ~MASK)
...
{
    unsigned long int* src64;
    unsigned long int* dst64;
    unsigned char* src8 = AS_U8(source);
    unsigned char* dst8 = AS_U8(dest);
    unsigned long int misalign = MISALIGN(dest);
    nbytes = nbytes - misalign;
    while (misalign--) {
        *dst8++ = *src8++;
    }
    src64 = AS_U64(src8);
    dst64 = AS_U64(dst8);
    misalign = nbytes % ALIGN;
    nbytes = nbytes - misalign;
    while (nbytes) {
        *dst64++ = *src64++;
        nbytes = nbytes - ALIGN;
